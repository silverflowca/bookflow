...
function ReaderAuthModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="bg-surface w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden border border-theme/50 max-h-[90vh] flex flex-col">

        {/* Branded header — compact horizontal layout */}
        <div className="relative bg-accent px-5 py-4 flex-shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <span className="text-lg font-bold text-white tracking-tight">BookFlow</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3 overflow-y-auto">
          {tab === 'register' && (
            <div>
              ...
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted pointer-events-none" />
                <input
                  type="text"
                  value={displayName}
                  ...
                  required
                  autoComplete="name"
                  className="w-full pl-8 pr-3 py-2 text-sm border border-theme rounded-lg bg-surface-hover focus:bg-surface focus:ring-2 focus:ring-accent focus:border-accent focus:outline-none transition-colors"
                />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div>
            ...
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted pointer-events-none" />
              <input
                ...
                className="w-full pl-8 pr-9 py-2 text-sm border border-theme rounded-lg bg-surface-hover focus:bg-surface focus:ring-2 focus:ring-accent focus:border-accent focus:outline-none transition-colors"
              />
              <button
                type="button"
                onClick={() => setShowPassword(v => !v)}
                tabIndex={-1}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted hover:text-theme transition-colors"
          ...
          {error && (
            <div className="flex items-start gap-2 p-2.5 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2 theme-button-primary rounded-lg font-semibold text-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
