...
-- Schema: bookflow
-- Run: docker exec -i supabase_db_silverflow psql -U postgres -d postgres < "bookflow/migrations/001_bookflow_schema.sql"
-- ============================================================================
...
-- ============================================================================
-- BOOK SETTINGS (Author configures reader permissions)
-- ============================================================================
...
-- ============================================================================
-- INLINE CONTENT (Universal table for all inline elements)
-- ============================================================================
CREATE TABLE IF NOT EXISTS bookflow.inline_content (
    ...
    content_type TEXT NOT NULL CHECK (content_type IN ('question', 'poll', 'highlight', 'note', 'link', 'audio', 'video')),
    start_offset INTEGER NOT NULL, -- Character position start
    end_offset INTEGER NOT NULL,   -- Character position end
    anchor_text TEXT, -- The text this content is attached to
    content_data JSONB NOT NULL, -- Type-specific data
    created_by UUID NOT NULL REFERENCES bookflow.profiles(id) ON DELETE CASCADE,
    is_author_content BOOLEAN DEFAULT FALSE, -- Author pre-added vs reader added
...
-- ============================================================================
-- FILE REFERENCES (Links to FileFlow)
-- ============================================================================
CREATE TABLE IF NOT EXISTS bookflow.file_references (
    ...
    fileflow_file_id UUID NOT NULL,
    file_type TEXT NOT NULL, -- 'cover', 'audio', 'video', 'document'
    display_name TEXT,
    file_url TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ============================================================================
-- CHANGE LOG (For ChangeFlow integration)
-- ============================================================================
CREATE TABLE IF NOT EXISTS bookflow.change_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_type TEXT NOT NULL, -- 'book', 'chapter', 'inline_content'
    ...
    changeflow_id TEXT, -- ID from ChangeFlow if synced
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Books: Public books readable by all, authors can manage own
...
-- Chapters: Inherit from book visibility
...
-- Inline Content: Visibility-based
...
-- Poll Responses: Users manage own
...
-- Question Answers: Users manage own
...
-- Reading Progress: Users manage own
...
-- File References: Inherit from book
...
-- Change Log: Author can see changes to their books
...
-- Auto-create book settings when book is created
...
-- Calculate word count and read time for chapters
...
-- Log changes for ChangeFlow integration
...
COMMENT ON TABLE bookflow.inline_content IS 'Questions, polls, highlights, notes, links, and media attached to text';
